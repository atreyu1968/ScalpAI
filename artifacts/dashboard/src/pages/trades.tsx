...
export default function TradesPage() {
  ...
  const exportCsv = useCallback(() => {
    ...
    const headers = ["ID", "Bot ID", "Par", "Lado", "Modo", "Estado", "Entrada", "Salida", "Cant.", "PnL", "Comisión", "Deslizamiento", "Señal IA", "Confianza", "TP1%", "TP2%", "TP3%", "Nivel TP", "PnL Parcial", "Apertura", "Cierre"];
    const rows = trades.map((t: TradeLogItem) => [
      t.id, t.botId, t.pair, t.side, t.mode, t.status, t.entryPrice, t.exitPrice || "", t.quantity,
      t.pnl || "", t.commission || "", t.slippage || "", t.aiSignal || "", t.aiConfidence || "",
      t.aiTp1Pct || "", t.aiTp2Pct || "", t.aiTp3Pct || "", t.tpLevelReached, t.realizedPnl || "", t.openedAt, t.closedAt || ""
    ]);
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-muted-foreground text-xs bg-muted/30">
                      ...
                      <th className="text-right py-3 px-3">TP1/TP2/TP3</th>
                      <th className="text-center py-3 px-3">Progreso TP</th>
                      ...
                      return (
                        <tr key={t.id} className="border-b border-muted/20 hover:bg-muted/10" data-testid={`trade-row-${t.id}`}>
                          ...
                          <td className="py-2 px-3 text-right font-mono text-xs text-yellow-400">
                            {t.aiTp1Pct ? (
                              <span>{t.aiTp1Pct}% / {t.aiTp2Pct}% / {t.aiTp3Pct}%</span>
                            ) : t.aiTakeProfitPct ? (
                              <span>{t.aiTakeProfitPct}%</span>
                            ) : "-"}
                          </td>
                          <td className="py-2 px-3 text-center">
                            {t.aiTp1Pct ? (
                              <div className="flex items-center justify-center gap-1">
                                <span className={`inline-block w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center ${t.tpLevelReached >= 1 ? "bg-emerald-500 text-white" : "bg-muted text-muted-foreground"}`}>1</span>
                                <span className={`inline-block w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center ${t.tpLevelReached >= 2 ? "bg-emerald-500 text-white" : "bg-muted text-muted-foreground"}`}>2</span>
                                <span className={`inline-block w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center ${t.tpLevelReached >= 3 ? "bg-emerald-500 text-white" : "bg-muted text-muted-foreground"}`}>3</span>
                              </div>
                            ) : "-"}
                          </td>
