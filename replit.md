...
- **Trading Strategies (Multi-Strategy Dispatcher):** Each bot has a `strategy` field (`ai` or `trend_pullback`, default `trend_pullback` for new bots). The signal provider in `index.ts` dispatches to the appropriate engine per cycle. Legacy bots with `null` strategy fall back to AI for safety. The `/api/ai/bot-phase/:botId` endpoint is **strategy-aware**: for `trend_pullback` bots it derives the badge phase/label from `getLastDecision(botId)` (warming_up reasons → "Calentando velas 4H/1H", limit-order pending → "Orden límite pendiente", filter rejections → "Analizando" with the specific filter as detail, open trade → "En operación"). For `ai` bots it keeps the historical pattern-engine flow (50×1m candles → ADX/EMA → scanning).
